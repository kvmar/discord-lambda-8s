...
queue_dao = QueueDao()
async def button_flow_tree(interaction: Interaction):
  if interaction.custom_id == QueueManager.join_queue_custom_id:
    await join_queue_button(interaction.guild_id, interaction)

async def join_queue_button(guild_id: str, inter: Interaction):
  ...
  await Bot.edit_message(message_id=record.message_id, channel_id=record.channel_id, guild_id=record.guild_id)
...
def post_leaderboard(queue_record: QueueRecord, inter: Interaction):
    ...
    sorted_player_list = sorted(player_list, key=lambda x: x.get_rating(), reverse=True)
    ...
    for user in sorted_player_list:
        user_data = list()
        print(f"Got player: {user.player_name} with elo {user.get_rating()}")
        ...
        user_data.append(str(int(float(user.get_rating()) * 100)))

...
def generate_match_done_embed(team1, team2, guild_id, queue_record: QueueRecord):
    ...
    for user in team1:
        ...
        player_str = str(player_data.get_rank_emoji()) + player_data.player_name + str(player_data.get_streak()) + " " + str(int(float(player_data.get_rating()) * 100)) + " (" + player_data.delta + f")\n"
...
def update_queue_embed(record: QueueRecord) -> ([Embedding], [Components]):
    if len(record.team_1) == 0 or len(record.team_2) == 0:
        ...
        for user in record.team_1:
            player_data = player_dao.get_player(record.guild_id, user)
            team1_str = team1_str + str(player_data.get_rank_emoji()) + player_data.player_name + str(player_data.get_streak()) + ": " + str(int(float(player_data.get_rating()) * 100)) + "\n"

        team2_str = "Team 2: \n"
        for user in record.team_2:
            player_data = player_dao.get_player(record.guild_id, user)
            team2_str = team2_str + str(player_data.get_rank_emoji()) + player_data.player_name + str(player_data.get_streak()) + ": " + str(int(float(player_data.get_rating()) * 100)) + "\n"

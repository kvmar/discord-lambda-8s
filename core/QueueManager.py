...
def create_queue_resources(guild_id: str, queue_name: str):
    ...
    embed = Embedding(title=f"🎮 Queue {queue_name}",
                      desc=f"Players: {len(response.queue)}/8",
                      color=0x7c3aed)
...
def update_queue_embed(record: QueueRecord) -> ([Embedding], [Components]):
    if len(record.team_1) == 0 or len(record.team_2) == 0:
        ...
        waitlist_component.add_button("Join Waitlist", f"join_waitlist#{record.queue_id}", False, 1)
        waitlist_component.add_button("Leave Waitlist", f"leave_waitlist#{record.queue_id}", False, 4)
...
def update_queue_view(record: QueueRecord, embeds: list[Embedding], components: list[Components], inter: Interaction):
    ...
    if  curr_time > record.expiry:
        ...
        for key, value in record.channel_config.items():
            resp = inter.edit_message(channel_id=key, message_id=value, embeds=embeds, components=components)
            print(f'Queue message_id: {resp}')
            update_message_id(inter.guild_id, resp[0], resp[1], record.queue_id)

...
def create_queue() -> Embedding:
    ...
    embed.add_button("Join queue", join_queue_custom_id, False)
    embed.add_button("Leave queue", leave_queue_custom_id, False)
    embed.add_button("Start queue", start_queue_custom_id, True)
    return embed